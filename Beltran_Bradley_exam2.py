...
def in_roots_children(T,k):
    ...
    if T.right.is_empty:
        if T.left.key is k:
            return True
    if T.left.key is k or T.right.key is k:
        return True
    return False
...
def num_descendants(T,k):
    ...
    if T.is_empty:
        return -1
    x=T.find(k) # k will stay same
    if x is None:
        return -1
    count=0
    if not T.is_empty:
        count=num_descendants(x.left,k)
        count+=1
        count=count+num_descendants(x.right,k)
    return count
...
def at_depth_d_or_less(T,d):
    ...
    L=[]
    if not T.is_empty:
        L=at_depth_d_or_less(T.left,d-1)
        if d>=0:
            L.append(T.key)
        L=L+at_depth_d_or_less(T.right,d-1)
    return L
