...
def brightest_region_naive(im, rw, rh):
    ...
    sum = 0
    brightest = 0
    for row in range(im.shape[0]-rh):
        for col in range(im.shape[1]-rw):
            for h in range(rh):
                for w in range(rw):
                    sum+=im[row+h,col+w]
                    if brightest<sum:
...
def brightest_region_integral_image(im, rw, rh):
    ...
    result_col = 0
    brightest = 0
    sum = np.zeros((im.shape+1, im[0].shape+1))
...
if __name__ == '__main__':
    ...
    # Integral Image
    start_time = timer()
    S = integral_image(im1)
    end_time = timer()
    print(f'Computing the integral image took {end_time-start_time:.3f} seconds to compute')
    
    for nrows, ncols in INTEGRAL_REGION_SIZES:
        start_time = timer()
        brow, bcol = brightest_region_integral_image(im1, S, nrows, ncols)
        end_time = timer()
